...
# Known Issues
### wrong api key lets you through, but will error when you use the chatbot_admin
...
def load_pdfs(reset=False):
    ...
    with open(f"data_json.json", 'r') as openfile:
        data_json = json.load(openfile)
    for root, dirs, files in os.walk(f"pdf_files"):
        for file in files:
    ...
    for file in pdf_files:
        ...
        if file not in data_json["stored_pdfs"].values():
            ...
            if len(data_json["stored_pdfs"].keys()) == 0:
                ...
                # Convert the keys to integers and find the maximum
                max_key = max(int(key) for key in data_json["stored_pdfs"].keys())
...
def chatbot(premise = "You’re a Chatbot that has access these information. The manual information is here: ",
            prompt_mod = ". Notes: Utilize the information available in the manuals to provide assistance",
            ...
            details = False,
            breadth = 1):
    '''Start of the chatbot_admin. Able to modify the premise or prompt'''
    global messages
    response_list = []
    load_pdfs()
    print(guide)
    def conversation(response):
        ...
        while True:
            ...
            try:
                ...
                openai.api_key = input("It seems like something's wrong with OpenAI. \nReenter your API key: ")
                return False
            except InvalidRequestError as a:
                print("Token exceeded. Clearing messages and retrying...")
                messages = [{"role": "system", "content": premise}]
                continue  # Continue the loop to retry the conversation with cleared messages
